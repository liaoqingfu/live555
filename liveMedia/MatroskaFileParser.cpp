    ...
    while (fCurOffsetInFile < fLimitOffsetInFile) {
        ...
        case MATROSKA_ID_TRACK_TYPE: {
            ...
            if (parseEBMLVal_unsigned(size, trackType) && track != NULL) {
                ...
                track->trackType
                    = trackType == 1 ? MATROSKA_TRACK_TYPE_VIDEO : trackType == 2 ? MATROSKA_TRACK_TYPE_AUDIO : trackType == 0x11 ? MATROSKA_TRACK_TYPE_SUBTITLE : MATROSKA_TRACK_TYPE_OTHER;
#ifdef DEBUG
                fprintf(stderr, "\tTrack Type 0x%02x (%s)\n", trackType,
                    track->trackType == MATROSKA_TRACK_TYPE_VIDEO ? "video" : track->trackType == MATROSKA_TRACK_TYPE_AUDIO ? "audio" : track->trackType == MATROSKA_TRACK_TYPE_SUBTITLE ? "subtitle" : "<other>");
#endif
            }
            break;
        }
                ...
                if (track != NULL) {
                    ...
                    if (track->codecID != NULL) {
                        if (strcmp(track->codecID, "V_MPEG4/ISO/AVC") == 0) { // H.264
                            ...
                            if (codecPrivateSize < 23 || (codecPrivate[13] & 0xF0) != 0xF0 || (codecPrivate[15] & 0xFC) != 0xFC || (codecPrivate[16] & 0xFC) != 0xFC || (codecPrivate[17] & 0xF8) != 0xF8 || (codecPrivate[18] & 0xF8) != 0xF8) {
...
typedef enum { NoLacing,
    XiphLacing,
    FixedSizeLacing,
    EBMLLacing } MatroskaLacingType;
