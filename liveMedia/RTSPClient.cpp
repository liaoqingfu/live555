    ...
    Boolean streamUsingTCP)
{
    ...
    do {
        ...
        fprintf(stderr, "lcp-debug RTSPClient::handleSETUPResponse transportParamsStr={%s}\n", transportParamsStr);
        ...
        if (streamUsingTCP) {
            ...
            if (subsession.rtpSource() != NULL) {
                fprintf(stderr, "RTSPClient::handleSETUPResponse, subsession.rtpChannelId=%d\n", subsession.rtpChannelId);
            ...
            if (subsession.rtcpInstance() != NULL) {
                fprintf(stderr, "RTSPClient::handleSETUPResponse, subsession.rtcpChannelId=%d\n", subsession.rtcpChannelId);
    ...
    char const* rangeParamsStr, char const* rtpInfoParamsStr)
{
    ...
    do {
        if (&session != NULL) {
            ...
            if (rangeParamsStr != NULL && !parseRangeParam(rangeParamsStr, session.playStartTime(), session.playEndTime(), session._absStartTime(), session._absEndTime(), startTimeIsNow))
                break;
            rangeOK = True;
            ...
            speedOK = True;
            Boolean startTimeIsNow;
            if (rangeParamsStr != NULL && !parseRangeParam(rangeParamsStr, subsession._playStartTime(), subsession._playEndTime(), subsession._absStartTime(), subsession._absEndTime(), startTimeIsNow))
                break;
            rangeOK = True;
            ...
            if (foundRequest != NULL) {
                ...
                if (responseCode == 200) {
                    ...
                    if (strcmp(foundRequest->commandName(), "SETUP") == 0) {
                        ...
                        if (!handlePLAYResponse(*foundRequest->session(), *foundRequest->subsession(), scaleParamsStr, speedParamsStr, rangeParamsStr, rtpInfoParamsStr))
                            break;
...
HandlerServerForREGISTERCommand ::HandlerServerForREGISTERCommand(UsageEnvironment& env, onRTSPClientCreationFunc* creationFunc, int ourSocket, Port ourPort,
    UserAuthenticationDatabase* authDatabase, int verbosityLevel, char const* applicationName)
