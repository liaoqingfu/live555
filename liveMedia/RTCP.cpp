...
static unsigned const maxRTCPPacketSize = 14560;
...
void RTCPInstance ::injectReport(u_int8_t const* packet, unsigned packetSize, struct sockaddr_in const& fromAddress)
{
...
{
    do {
        envir() << "fNumBytesAlreadyRead =" << fNumBytesAlreadyRead << ", maxRTCPPacketSize =" << maxRTCPPacketSize << "\n";
            ...
            case RTCP_PT_SDES: {
                ...
                while (length >= 8) { // A valid chunk must be at least 8 bytes long
                    ...
                    while (itemType != 0) {
                        ...
                        fprintf(stderr, "\t\t%s:%s\n",
                            itemType == 1 ? "CNAME" : itemType == 2 ? "NAME" : itemType == 3 ? "EMAIL" : itemType == 4 ? "PHONE" : itemType == 5 ? "LOC" : itemType == 6 ? "TOOL" : itemType == 7 ? "NOTE" : itemType == 8 ? "PRIV" : "(unknown)",
