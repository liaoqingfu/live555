    ...
    unsigned int bufsize, unsigned int prerecordsecs, unsigned int createsharememory)
{
    ...
    wchar_t wszFramelistName[36] = {
        0,
    };
    wchar_t wszDataName[36] = {
        0,
    };
    if (NULL == pObj)
        return -1;
    if (createsharememory == 0x01 && bufsize < 1)
        return -1;
    if ((sharememory == 0x01) && (NULL == sharename || (0 == wcscmp(sharename, TEXT("\0")))))
        return -1;
...
int SSQ_AddData(SS_QUEUE_OBJ_T* pObj, unsigned int channelid, unsigned int mediatype, MEDIA_FRAME_INFO* frameinfo, char* pbuf)
{
    ...
    if (NULL == pObj || NULL == frameinfo || NULL == pbuf)
        return -1;
    if (NULL == pObj->pQueData)
        return -1;
    if (NULL == pObj->pQueHeader)
    ...
    if ((pObj->pQueHeader->writepos + sizeof(SS_BUF_T) + frameinfo->length) <= pObj->pQueHeader->bufsize) {
        ...
        if (mediatype == MEDIA_TYPE_VIDEO)
            SSQ_AddFrameInfo(pObj, pObj->pQueHeader->writepos, frameinfo);
    ...
    else // if (pObj->pQueHeader->bufsize - pObj->pQueHeader->writepos >= (int)(frameinfo->length+sizeof(SS_BUF_T)))	//从尾写到头
    {
        ...
        if (remain >= sizeof(SS_BUF_T)) {
            ...
            //记录帧位置
            if (mediatype == MEDIA_TYPE_VIDEO)
                SSQ_AddFrameInfo(pObj, nAdd, frameinfo);
...
int SSQ_GetData(SS_QUEUE_OBJ_T* pObj, unsigned int* channelid, unsigned int* mediatype, MEDIA_FRAME_INFO* frameinfo, char* pbuf)
{
    ...
    if ((pObj->pQueHeader->readpos + sizeof(SS_BUF_T)) <= pObj->pQueHeader->bufsize) {
...
#ifdef _WIN32
                while (pObj->pQueHeader->clear_flag != 0x00) {
                    Sleep(1);
                }
#else
                while (pObj->pQueHeader->clear_flag != 0x00) {
        ...
        if (NULL != mediatype)
            *mediatype = pNode->mediatype;
        if (NULL != channelid)
            *channelid = pNode->channelid;
        ...
        if ((pObj->pQueHeader->readpos + pNode->frameinfo.length + sizeof(SS_BUF_T)) <= pObj->pQueHeader->bufsize) {
            ...
            if (remain >= sizeof(SS_BUF_T)) {
                ...
                if (remain > 0) {
                    ...
                    if (NULL != pbuf)
                        memcpy(pbuf, pObj->pQueData + pObj->pQueHeader->readpos, remain);
                    //memset(pObj->pQueData+pObj->pQueHeader->readpos, 0x00, remain);	//clear
                    if (NULL != pbuf)
                        memcpy(pbuf + remain, pObj->pQueData, frameinfo->length - remain);
                    ...
                    if (remain < 0) {
                        ...
                        SSQ_TRACE("111尾有部分数据... 首有部分数据... remain<=0: %d\n", remain);
                        if (NULL != pbuf)
                            memcpy(pbuf, pObj->pQueData, frameinfo->length);
                        //memset(pObj->pQueData, 0x00, frameinfo->length-remain);	//clear
                        pObj->pQueHeader->readpos = frameinfo->length;
                        pObj->pQueHeader->totalsize -= frameinfo->length;
                        pObj->pQueHeader->totalsize -= sizeof(SS_BUF_T);
        ...
        if (MEDIA_TYPE_VIDEO == pNode->mediatype)
            pObj->pQueHeader->videoframes--;
        ...
        if (remain > 0) {
            ...
            if (NULL != mediatype)
                *mediatype = bufnode.mediatype;
            //SSQ_TRACE("3 frame length: %d\n", bufnode.frameinfo.length);
            if (NULL != pbuf)
                memcpy(pbuf, pObj->pQueData + pObj->pQueHeader->readpos, bufnode.frameinfo.length);
            ...
            if (MEDIA_TYPE_VIDEO == bufnode.mediatype)
                pObj->pQueHeader->videoframes--;
        } else {
    ...
    if ((*pOffset + sizeof(SS_BUF_T)) <= pObj->pQueHeader->bufsize) {
        ...
        if ((*pOffset + pNode->frameinfo.length + sizeof(SS_BUF_T)) <= pObj->pQueHeader->bufsize) {
            ...
            *pOffset += sizeof(SS_BUF_T);
            ...
            if (NULL != pbuf)
                memcpy(pbuf, pObj->pQueData + *pOffset, frameinfo->length);
            ...
            *pTotalSize -= (frameinfo->length + sizeof(SS_BUF_T));
            ...
            if (remain >= sizeof(SS_BUF_T)) {
                *pOffset += sizeof(SS_BUF_T);
                ...
                if (remain > 0) {
                    if (NULL != pbuf)
                        memcpy(pbuf, pObj->pQueData + *pOffset, remain);
                    if (NULL != pbuf)
                        memcpy(pbuf + remain, pObj->pQueData, frameinfo->length - remain);
        ...
        if (MEDIA_TYPE_VIDEO == pNode->mediatype && clearflag == 0x01)
            pObj->pQueHeader->videoframes--;
    } else {
        ...
        if (remain > 0) {
            ...
            *pOffset = sizeof(SS_BUF_T) - remain;
            ...
            if (NULL != pbuf)
                memcpy(pbuf, pObj->pQueData + *pOffset, bufnode.frameinfo.length);
            *pOffset += bufnode.frameinfo.length;
            *pTotalSize -= (frameinfo->length + sizeof(SS_BUF_T));

            SSQ_TRACE("[SSQ_GetDataByPosition]GET DATA OK..\n");
            if (MEDIA_TYPE_VIDEO == bufnode.mediatype && clearflag == 0x01)
                pObj->pQueHeader->videoframes--;
        } else {
